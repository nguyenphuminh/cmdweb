...
for (const route of routes) {
    const [ method, url, callback, format ] = route.split(" ");
...
const server = http.createServer(async (req, res) => {
    ...
    if (handlers[method]) {
        ...
        const callback = handlers[method][url].callback;
        // Get the message format of request
        const format = handlers[method][url].format;
        ...
        for (const header in req.headers) {
            reqInfo += `set req.headers["${header}"]=${secureBatchString(req.headers[header])}\n`;
        }
